    ...
    try {
        ...
        if (subcommand === 'ironman_sweats') {
            ...
            if (skyblock_xp < IMS_req*100) {
                ...
                await interaction.reply({ content: `You must be Skyblock Level ${IMS_req} to apply for IMS`, ephemeral: true });
                return;
            }
            await interaction.reply({ content: 'You have not successfully applied for Ironman Sweats (not implemented yet)', ephemeral: true });
        }
            ...
            if (skyblock_xp < IMC_req*100) {
                console.log(`    Not enough skyblock xp: ${skyblock_xp}`);
                await interaction.reply({ content: `You must be Skyblock Level ${IMC_req} to apply for IMC`, ephemeral: true });
                return;
            }
            await interaction.reply({ content: 'You have not successfully applied for Ironman Casuals (not implemented yet)', ephemeral: true });
        }
            ...
            if (skyblock_xp < IMA_req*100) {
                console.log(`    Not enough skyblock xp: ${skyblock_xp}`);
                await interaction.reply({ content: `You must be Skyblock Level ${IMA_req} to apply for IMA`, ephemeral: true });
                return;
            }
            await interaction.reply({ content: 'You have not successfully applied for Ironman Academy (not implemented yet)', ephemeral: true });
        }    
    }

    catch (error) {
        console.error('Error applying for guild:', error);
        await interaction.reply({ content: `An error occurred while applying for a guild: ${error.message}`});
    }
}
