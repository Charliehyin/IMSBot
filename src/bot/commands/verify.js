...
const verify_interaction = async (interaction, db) => {
	...
	try {
		...
		if (verified === true) {
			...
			if (
				guild.members.me.roles.highest.comparePositionTo(member.roles.highest) >
				0
			) {
			...
			await interaction.reply(
				`Successfully verified \`${discord_username}\` with IGN \`${ign}\``,
			);
		} else {
			console.log(
		...
		await interaction.reply(
			`There was an error while trying to verify the user: ${error.message}`,
		);
	}
};
